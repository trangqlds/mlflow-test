...
def _patch_sklearn_check_X_y() -> callable:
    """Monkeypatch sklearn.check_X_y to ignore force_all_finite for compatibility."""
    try:
        from sklearn.utils import validation

        original = validation.check_X_y

        def patched_check_X_y(*args, **kwargs):
            kwargs.pop('force_all_finite', None)
            return original(*args, **kwargs)

        validation.check_X_y = patched_check_X_y
        logger.debug('Patched sklearn.utils.validation.check_X_y to ignore force_all_finite')
        return original
    except Exception as e:
        logger.warning('Could not patch sklearn.check_X_y: %s', e)
        return None


def _restore_sklearn_check_X_y(original):
    try:
        if original is not None:
            from sklearn.utils import validation
            validation.check_X_y = original
            logger.debug('Restored original sklearn.utils.validation.check_X_y')
    except Exception as e:
        logger.warning('Could not restore sklearn.check_X_y: %s', e)
...
def train_lgbm(X_train: np.ndarray, y_train: np.ndarray, learning_rate: float, max_depth: int, n_estimators: int) -> lgb.LGBMClassifier:
    ...
    try:
        ...
        if 'force_all_finite' in str(e):
            logger.warning('Compatibility issue detected: %s. Attempting sklearn check_X_y patch', e)
            original_check = _patch_sklearn_check_X_y()
            try:
                best_model.fit(X_train, y_train)
                logger.debug('LightGBM model training completed after patching check_X_y')
                return best_model
            finally:
                _restore_sklearn_check_X_y(original_check)
